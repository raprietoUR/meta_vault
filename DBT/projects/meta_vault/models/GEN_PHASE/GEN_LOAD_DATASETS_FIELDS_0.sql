...
SELECT TMP.COD_DATASET,TMP.ID_ORDER,TMP.COD_DATASET_FIELD,TMP.COD_TYPE,TMP.COD_COLUMN_NAME_TARGET, TMP.DESC_BUSINESS_DEFINITION, '1' SW_TRACK_DIFF, '0' SW_ACTIVE_VALUE_DEFAULT, '' COLUMN_VALUE_DEFAULT_FX, '0' SW_VIRTUAL_FIELD_VALUE, ' Business Description' DESC_BUSINESS_DESCRIPTION
, 0 SW_CLUSTERIZABLE
...
, COL.COLUMN_NAME "DESC", '1' SW_TRACK_DIFF, '0' SW_ACTIVE_VALUE_DEFAULT, '' COLUMN_VALUE_DEFAULT_FX, '0' SW_VIRTUAL_FIELD_VALUE, COALESCE(COL.COMMENT,COL.COLUMN_NAME)|| ' Business Description' DESC_BUSINESS_DESCRIPTION
, 0 SW_CLUSTERIZABLE
...
INNER JOIN  {{ ref('GEN_REFRESH_MODEL_LOAD_RUNS_LAST') }} M
 ...
 LEFT JOIN {{ ref('RAW_LOAD_RELATIONSHIPS') }} REL_1  		ON DS.COD_DATASET = REL_1.REFERENCE_COD_DATASET AND COL.COLUMN_NAME = REL_1.REFERENCE_COD_NAME AND DS.ID_MODEL_RUN = REL_1.ID_MODEL_RUN
 LEFT JOIN {{ ref('RAW_LOAD_RELATIONSHIPS') }} REL_2  		ON DS.COD_DATASET = REL_2.TARGET_COD_DATASET  AND COL.COLUMN_NAME = REL_2.TARGET_COD_NAME AND DS.ID_MODEL_RUN = REL_2.ID_MODEL_RUN
WHERE
     M.COD_TYPE_RUN='{{ var('cod_type_run') }}'
 AND M.COD_MODEL= '{{ var('cod_model') }}'
